...
class ChannelPipeline {
  ...
  async runLoop () {
    ...
    // Forward H.264 profile/level to muxer so PMT carries the proper AVC descriptor
    // (VLC/MX/Kodi/TiviMate need this to select an H.264 decoder; without it many
    // STBs/players see audio but show a black screen on video).
    if (vInfo?.avcC) {
      const avcC = vInfo.avcC;
      this.muxer.setVideoInfo({
        profileIdc: avcC[1],
        constraintSet: avcC[2],
        levelIdc: avcC[3],
      });
    }
